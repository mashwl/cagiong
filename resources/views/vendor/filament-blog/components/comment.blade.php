...
<form action="{{ route('filamentblog.comment.store', ['post' => $post->slug]) }}" method="POST" id="comments">
    ...
    <div class="border-t-2 py-10">
        <div class="mb-7">
            <h3 class="mb-2 text-2xl font-semibold">Để Lại Ý Kiến Của Bạn </h3>
        </div>
        <div class="mb-6">
            <label class="mb-2 block text-sm font-semibold" for="author-comment">Bình Luận *</label>
            <textarea name="comment" rows="4" placeholder="Viết bình luận của bạn ở đây" type="text" id="author-comment"
        ...
        <div>
            @if(auth()->user()?->canComment())
                <button type="submit"
                        class="g-recaptcha bg-primary-600 hover:bg-primary-700 rounded-lg px-8 py-2 font-semibold text-white transition-all duration-300"
            ...
            @else
                <a
                        ...
                        class="bg-primary-600 hover:bg-primary-700 rounded-lg px-8 py-2 font-semibold text-white transition-all duration-300">
                    <span>Đăng Nhập</span>
                </a>
            @endif
        </div>
